...
ptm <- proc.time()
DF <- read_csv("./yellow-trip-data/Yellow_Sample.csv", col_names=TRUE)
READR_READ_TIME <- (proc.time() - ptm)
READR_READ_TIME

# The original data
data <- read_csv("./yellow-trip-data/yellow_tripdata_2017-06.csv")
...
cat("If we had processed the whole file, would have taken",
    as.numeric(LOOP_TIME[3]*1000/60, units = "hours"),"hours.")
...
ptm <- proc.time()
DF <- mutate(DF,trip_length2=(tpep_dropoff_datetime - tpep_pickup_datetime))
MUTATE_TIME <- (proc.time() - ptm)
MUTATE_TIME

View(DF)

data$trip_length3 <- (data$tpep_dropoff_datetime - data$tpep_pickup_datetime)

trip_length3 <- (tpep_dropoff_datetime - tpep_pickup_datetime)
